...
XLSFile   = 'DataFiles\Xlsx\Tables_07_11.xlsx';
...
for i = 1:nMM
    ...
    if i == 1
        ...
        for j = 1:nMETRICS
            TAB8a(i,j) = util_htest(met(:,j),bench_all(:,j),ALPHA);
        end
    end
end
[~,ORD] = sort(TAB8(:,nMETRICS),'descend');
TAB8S = TAB8(ORD,:);
TAB8aS = util_H0_2_H0str(TAB8a(ORD,:),TAB8S(:,MET_TAB));
MLabS = MLabels(ORD);
...
%% Save Slding Windows Info
DateLabels = { ...
    datestr(SW_Dates(   1,2),'dd/mmm/yy') ...
    datestr(SW_Dates(NSWD,2),'dd/mmm/yy') ...      
    };
save([InPath 'Sliding_Windows.mat'],'METR','DateLabels','NSWD');